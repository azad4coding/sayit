...
export default function NotificationsPage() {
  ...
  function NotifRow({ n, last }: { n: Notification; last: boolean }) {
    return (
      <button
        onClick={() => handleTap(n)}
        className={`w-full flex items-start gap-3 px-4 py-4 text-left active:bg-gray-50 transition-colors relative ${!last ? "border-b border-gray-50" : ""}`}
        style={{ background: n.isNew ? "rgba(255,107,138,0.04)" : "white" }}
      >
        {/* Unread pink dot */}
        {n.isNew && (
