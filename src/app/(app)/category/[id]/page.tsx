  ...
  return (
    <div className="flex flex-col min-h-dvh bg-gray-50">
      ...
      <div className="relative overflow-hidden"
        ...
        {/* Back button + left-aligned title */}
        <div className="relative z-10 flex items-center gap-3">
          ...
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "white", margin: 0 }}>{category.name}</h1>
        </div>
      </div>
...
function PawMomentsPage({ gradient_from, gradient_to }: { gradient_from: string; gradient_to: string }) {
  ...
  return (
    <div className="flex flex-col min-h-dvh" style={{ background: "linear-gradient(160deg,#F5EDE0,#EDD9C0)" }}>
      {/* Header */}
      <div className="relative overflow-hidden px-4 pb-5"
        style={{ background: `linear-gradient(135deg,${gradient_from},${gradient_to})`, paddingTop: "calc(env(safe-area-inset-top, 44px) + 12px)" }}>
        <div className="absolute top-0 right-0 w-40 h-40 rounded-full bg-white/10 -translate-y-1/2 translate-x-1/2" />
        <div className="absolute top-10 right-10 w-24 h-24 rounded-full bg-white/10" />
        {/* Back button + icon + left-aligned title (no "Special category" label) */}
        <div className="relative z-10 flex items-center gap-3">
          <button onClick={() => router.back()}
            style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(255,255,255,0.2)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
